...
class PyGameTextRenderer(TextRenderingMixin):
    ...
    def _get_empty_surface_for_generation(self, max_pixels_len) -> pygame.Surface:
        """
        Create and return an empty surface that we will later render the text to
        ...
        frame = (max_pixels_len, self.pixels_per_patch)
        surface = pygame.Surface(frame)
        surface.fill(pygame.color.THECOLORS[self.background_color])
        return surface
    ...
    def _render_text_to_surface_without_padding(self, text: str) -> Encoding:
        """
        Renders a single piece of text, e.g. a sentence or paragraph, to a surface and keeps track of
        how many patches in the rendered surface contain text without padding
        ...
        Returns:
            An Encoding of type `Encoding` containing the rendered text and metadata
        """
        line_width = self.font.get_rect(text).width
        surface = self._get_empty_surface_for_generation(line_width)
        sep_patches = []
    ...
    def __call__(
        ...
        **kwargs: Any
        ...
        if isinstance(text, list):
            ...
            padding = kwargs.get("generation", False)
            if padding is True:
                rendering_fn = self._render_text_to_surface_without_padding
            else:
