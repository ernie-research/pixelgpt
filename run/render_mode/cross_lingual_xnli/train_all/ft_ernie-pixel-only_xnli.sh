...
do
    ...
    do
        for MAX_STEPS in 30000
            do  
                RUN_NAME="experiment/render_mode/xnli_train_all/ernie-pixel-only/${TASK}-$(basename ${MODEL})-${RENDER_MODE}-${RENDERING_BACKEND}-${MODALITY}-${SEQ_LEN}-${BSZ}-${GRAD_ACCUM}-${NUM_NODE}-${LR}-${MAX_STEPS}-${SEED}"
