...
do
    ...
    do
        for MAX_STEPS in 250
            do
                RUN_NAME="experiment/render_mode/nlu_glue/ernie-pixel-only/${TASK}-$(basename ${MODEL})-${RENDER_MODE}-${RENDERING_BACKEND}-${MODALITY}-${SEQ_LEN}-${BSZ}-${GRAD_ACCUM}-${NUM_NODE}-${LR}-${MAX_STEPS}-${SEED}"
