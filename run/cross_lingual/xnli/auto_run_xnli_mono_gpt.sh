...
LOG_DIR=log/cross_lingual/xnli/train_all/$PT_TYPE/$CKPT_NAME
mkdir -p $LOG_DIR
bash run/cross_lingual/xnli/train_all/$PT_TYPE/ft_${PT_TYPE}_xnli_pair.sh $MODEL > $LOG_DIR/${CKPT_NAME}_pair.log 2>&1
sleep 60

# ===== train-en; ft pair =====
LOG_DIR=log/cross_lingual/xnli/train_en/$PT_TYPE/$CKPT_NAME
mkdir -p $LOG_DIR
bash run/cross_lingual/xnli/train_en/$PT_TYPE/ft_${PT_TYPE}_xnli_pair.sh $MODEL > $LOG_DIR/${CKPT_NAME}_pair.log 2>&1
sleep 60
